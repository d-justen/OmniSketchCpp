...
def read_and_process_csv(file_path):
    ...
    print(f"{os.path.basename(file_path)}: Filtered out {filtered_count} rows with QErr == 0")
    grouped = df.groupby('number_of_joins')['QErr'].apply(list)
    return grouped
...
def plot_boxplot(all_data, labels):
    ...
    ax.set_ylabel('QErr (log scale)')
    ax.set_yscale('log')
    ax.set_title('Box Plot of QErr by Number of Joins (per CSV)')
    ...
    plt.tight_layout()
    plt.savefig("ssb_skew_q_errs.pdf")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python plot_qerr.py <csv1> <csv2> ...")
        sys.exit(1)
