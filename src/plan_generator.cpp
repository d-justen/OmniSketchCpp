...
bool PlanGenerator::CheckIfResolvable(const std::shared_ptr<PlanExecItem>& item) const {
    ...
    for (const auto& fk_side_item_pair : item->probes_into) {
        const auto fk_side_item_it = plan_items.find(fk_side_item_pair.first);
        if (fk_side_item_it == plan_items.end()) {
            continue;  // Skip if item not found
        }

        const auto& fk_side_item = fk_side_item_it->second;
...
std::unordered_map<std::string, std::shared_ptr<PlanExecItem>> PlanGenerator::CreateInitialExecutionPlan() const {
    ...
    for (const auto& plan_item_pair : plan_items) {
        ...
        const auto& plan_item = plan_item_pair.second;
...
std::unordered_map<std::string, std::shared_ptr<PlanExecItem>> PlanGenerator::AddPredicatesToPlan(
    const std::unordered_map<std::string, std::shared_ptr<PlanExecItem>>& before) const {
    auto& registry = Registry::Get();
    std::unordered_map<std::string, std::shared_ptr<PlanExecItem>> after;
