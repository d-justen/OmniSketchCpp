...
public:
    ...
    virtual std::shared_ptr<OmniSketchCell> Execute(
        size_t max_sample_count, const std::vector<std::vector<std::shared_ptr<OmniSketchCell>>>& cells) const = 0;
};

class ProbeAllSum : public SetMembershipAlgorithm {
    std::shared_ptr<OmniSketchCell> Execute(
