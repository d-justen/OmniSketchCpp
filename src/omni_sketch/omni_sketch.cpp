...
PointOmniSketch::PointOmniSketch(size_t width_p, size_t depth_p, size_t max_sample_count_p,
    ...
    : width(width_p),
      depth(depth_p),
      max_sample_count(max_sample_count_p),
      set_membership_algo(std::move(set_membership_algo_p)),
      hash_processor(std::move(hash_processor_p)) {
    cells.resize(depth);
    for (auto& row : cells) {
        row.reserve(width);
...
std::shared_ptr<OmniSketchCell> PointOmniSketch::ProbeHash(uint64_t hash,
