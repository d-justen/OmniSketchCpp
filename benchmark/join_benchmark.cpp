...
enum class BenchmarkName : int64_t { 
	SSB = 0, 
	SSB_SKEW = 1, 
	JOB_LIGHT = 2 
};

enum class CombinatorType : int64_t { 
	EXHAUSTIVE = 0, 
	EXHAUSTIVE_CORRELATED = 1 
};
...
int64_t GetSSBQueryIndex(int64_t queryId) {
	switch (queryId) {
	case 11: return 0;
	case 12: return 1;
	case 13: return 2;
...
std::vector<int64_t> GetAllSSBSubQueries(size_t joinCount) {
	size_t offset = 0;
	size_t queryCount = 0;
	
	switch (joinCount) {
	case 0: queryCount = 23; break;
	case 1: offset = 23; queryCount = 21; break;
	case 2: offset = 44; queryCount = 34; break;
	case 3: offset = 78; queryCount = 18; break;
	case 4: offset = 96; queryCount = 3; break;
	default:
		throw std::logic_error("Invalid join count: " + std::to_string(joinCount));
	}
...
public:
	...
	std::vector<CountQuery> LoadSSBSkewQueries() {
		...
		const std::vector<std::string> queryFiles = {
			"../data/ssb-skew-sf1/queries.csv",
			"../data/ssb-skew-sf1/sub_queries_1.csv",
			"../data/ssb-skew-sf1/sub_queries_2.csv",
			"../data/ssb-skew-sf1/sub_queries_3.csv",
			"../data/ssb-skew-sf1/sub_queries_4.csv",
			"../data/ssb-skew-sf1/sub_queries_5.csv"
		};
		
		for (const auto& queryFile : queryFiles) {
			auto fileQueries = CSVImporter::ImportQueries(queryFile);
			queries.insert(queries.end(), 
			              std::make_move_iterator(fileQueries.begin()),
			              std::make_move_iterator(fileQueries.end()));
		}
...
private:
	...
	bool ssbSkewLoaded = false;
	bool jobLightLoaded = false;
};
