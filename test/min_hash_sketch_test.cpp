...
#include "include/min_hash_sketch/min_hash_sketch.hpp"
...
TEST_F(MinHashSketchSet, Intersection) {
	...
	auto *typed_result = dynamic_cast<omnisketch::MinHashSketchVector *>(result.get());
	EXPECT_NE(typed_result, nullptr);
	for (size_t i = 0; i < MATCH_COUNT; i++) {
		uint64_t hash = hf->HashRid(i);
		EXPECT_NE(std::find(typed_result->Data().begin(), typed_result->Data().end(), hash), typed_result->Data().end());
	}
}
